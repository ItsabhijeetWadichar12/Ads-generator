...
function Script({ videoData, onHandleInputChange }) {
    // Get the default script content safely
    const defaultScript = videoData?.script || videoData?.scriptVariant?.[0]?.content || '';
    ...
    return (
        <div className='p-6 shadow-lg rounded-2xl bg-slate-800/70 text-white backdrop-blur-lg border border-slate-700'>
            ...
            <div className=''>
                <label className='text-slate-400'>Video Project Topic</label>
                <Input
            ...
            <div className='mt-4'>
                <label className='text-slate-400'>Video Script</label>
                <Textarea
                    className={'text-lg bg-slate-700/50 border-slate-600 text-white rounded-lg h-40 mt-2 focus:ring-2 focus:ring-blue-500'}
                    onChange={(e) => onHandleInputChange('script', e.target.value)}
                    value={defaultScript}
                />
            </div>

            {videoData?.scriptVariant && videoData.scriptVariant.length > 0 && (
                ...
                ${script?.content === videoData?.script ? 'border-blue-500 bg-blue-900/50 text-white' : 'border-slate-700'}`}
                        ...
                        >
                            <h2 className='line-clamp-3'>{script?.content || 'No content available'}</h2>
                        </div>
                    ))}
                </div>
            )}
